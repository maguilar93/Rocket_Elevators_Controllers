  ...
  closeDoors() {
    while (this.openDoors) {
      if (this.obstacles) {
        ...
        this.openDoors = setTimeout(this.close, 3000);
      }
      break;
    }
  }
  ...
  requestFloor(requestedFloor) {
    ...
    for (var i = 0; i < this.requests.length; i++) {
      if (this.current_position > this.requests[i]) {
        this.closeDoors();
        for (
          var floor = this.current_position;
          floor > this.requests[i];
          floor--
        ) {
          this.moveDown();
          this.movement = "down";
          this.openDoors == true;
          console.log(elevator_2.current_position);
        }
        ...
        for (
          ...
          this.movement = "up";
          this.openDoors == true;
          console.log(elevator_2.movement);
          console.log(elevator_2.current_position);
        }
      } else {
        this.movement = "idle";
        this.requests.pop();
        console.log(elevator_2.movement);
        console.log(elevator_2.openDoors);
      }
    }
  }
...
function sendElev(elevator, requestedFloor, direction) {
  ...
  for (var i = 0; i < elevator.requests.length; i++) {
    ...
    if (elevator.current_position > elevator.requests[i]) {
      for (
        var floor = elevator.current_position;
        floor > elevator.requests[i];
        floor--
      ) {
        elevator.moveDown();
...
elevator_2.movement = "up";
elevator_1.current_position = 10;
elevator_2.current_position = 3;
listElevators.push(elevator_1);
listElevators.push(elevator_2);

requestElevator(3, "down");
// elevator_2.requestFloor(2);
// requestElevator(10, "down");
// elevator_2.requestFloor(3);
